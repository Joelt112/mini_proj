...
import { handleHashScroll } from "@/lib/scroll-util"
...
export default function HowItWorks() {
  ...
  return (
    <section
      ...
      id="how-it-works"
      className={cn(
        "py-16 md:py-24 bg-black text-white relative overflow-hidden scroll-mt-20 transition-all duration-1000",
        isHighlighted && "highlight-section",
      )}
    >
      {/* Navigation indicator that appears when scrolled to this section */}
      <div
        className={cn(
          "fixed top-20 left-1/2 transform -translate-x-1/2 bg-primary text-white px-4 py-2 rounded-full z-50 transition-all duration-500 flex items-center gap-2",
          isHighlighted ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-full pointer-events-none",
        )}
      >
        <CheckCircle className="h-4 w-4" />
        <span className="text-sm font-medium">How It Works Section</span>
      </div>
      ...
      <div className="container relative z-10 px-4 sm:px-6">
        ...
        >
          ...
          <p className="text-gray-300 max-w-2xl mx-auto text-base sm:text-lg">
            Bidding on Bidout is simple, secure, and exciting. Follow these steps to start your bidding journey.
          </p>
        </div>
